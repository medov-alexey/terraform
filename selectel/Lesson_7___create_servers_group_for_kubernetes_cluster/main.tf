...
# Инфраструктура L3: Определяем внутреннюю подсеть серверов. 
# Используем диапазон 10.10.0.0/24, чтобы избежать конфликтов с сетевыми плагинами Kubernetes (Calico/Cilium)
...
# Выводим подробную карту сетевых адресов серверов (внешний для SSH и внутренний для Kubespray)
output "assigned_ip_addresses" {
  value = {
    for name, instance in module.virtual_machines : name => {
