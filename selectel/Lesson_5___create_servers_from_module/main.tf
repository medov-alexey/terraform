...
# Итеративный вызов шаблона (модуля) для параллельного развертывания заданной группы серверов
module "virtual_machines" {
  source = "./modules/vps"

  # Передаем список серверов из файла vps_list.tf
  ...
  # ПЕРЕДАЕМ ИМЯ КЛЮЧА: берем имя из ресурса selectel_vpc_keypair_v2.ssh_key
  key_pair_name      = selectel_vpc_keypair_v2.ssh_key.name

  # ПЕРЕДАЕМ ID флейвора внутрь модуля
  flavor_id          = openstack_compute_flavor_v2.custom_tiny_flavor.id
}

# Вывод итоговой карты распределения IP-адресов в консоль в удобном для чтения формате "имя_сервера = ip"
